...
def detect_all_regions(gray: np.ndarray, expand_pixels: int = 10):
    ...
    H, W = gray.shape[:2]
    half_req = 64 // 2  # ventana base centrada
...
def main():
    ...
    ap.add_argument("--expand", type=int, default=10, help="Expansión de bbox (px) antes de centrar ventana 256")
    ...
    for idx, (x1,y1,x2,y2) in enumerate(rois):
        ...
        if input_mode == 'stack':
            roi256 = crop_center(image, cx, cy, 64) # Para imagen 2
            roi384 = crop_center(image, cx, cy, 128) # Para imagen 2
            #roi256 = crop_center(image, cx, cy, 192) # Para imagen 7
            #roi384 = crop_center(image, cx, cy, 384) # Para imagen 7
